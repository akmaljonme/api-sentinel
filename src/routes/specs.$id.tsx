...
export const Route = createFileRoute("/specs/$id")({
  head: ({ params }) => ({ meta: [{ title: `View spec — Flowt` }, { name: "description", content: "View OpenAPI spec and live mock." }] }),
  component: SpecPage,
});
        ...
        <aside className="border-r border-border bg-surface/50">
          <div className="sticky top-14 p-3">
            ...
            <div className="mt-2 space-y-4">
              ...
              {!endpoints.length && (
                <div className="text-[12px] text-text-muted px-1">No endpoints analyzed yet.</div>
              )}
            </div>
          </div>
        </aside>
            ...
            <>
              ...
              <div className="mt-6 flex items-center gap-1 border-b border-border">
                {(["Overview", "Try it", "Live"] as const).map((t) => (
                  ...
                  >
                    {t === "Live" ? "Live stream" : t === "Try it" ? "Try it" : "Overview"}
              ...
              {tab === "Live" && (
                mockServerId ? <div className="mt-6"><LiveRequests mockServerId={mockServerId} /></div>
                  : <div className="mt-6 text-[13px] text-text-muted">Mock server not yet ready.</div>
              )}
            </>
          ) : (
        ...
        <aside className="border-l border-border bg-surface/50">
          <div className="sticky top-14 p-4 space-y-4">
            <div>
              ...
              <div className="mt-1 text-[10.5px] text-text-muted">Send real HTTP requests to endpoints generated from your schema.</div>
            </div>
            {mockServerId ? (
              <>
                ...
                <div className="text-[10.5px] text-text-muted">
                  Public mock URL. Requests will be logged live to this workspace.
                </div>
              </>
            ) : (
              <div className="text-[12px] text-text-muted">Mock server not yet ready.</div>
            )}
          </div>
        </aside>
      </div>
      ...
      {resp && (
        <div className="rounded-lg border border-border bg-surface overflow-hidden">
          <div className="flex items-center gap-3 border-b border-border px-3.5 py-2 text-[12px]">
            ...
            <span className="ml-auto text-text-muted">Live response from mock server</span>
          </div>
