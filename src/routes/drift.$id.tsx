...
export const Route = createFileRoute("/drift/$id")({
  head: () => ({ meta: [{ title: "Drift hisoboti — Flowt" }, { name: "description", content: "Spec drift hisoboti va sinovchi o'zgarishlar tarixi." }] }),
  component: DriftPage,
});
...
function DriftPage() {
  ...
  if (loading) {
    return (
      <DashboardLayout crumbs={["Boshqaruv paneli", "Drift"]} action={false}>
  ...
  return (
    <DashboardLayout crumbs={["Boshqaruv paneli", "Drift", spec.name]} action={false}>
      <div className="mx-auto max-w-7xl p-6 lg:p-8 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <Link to="/specs/$id" params={{ id: spec.id }} className="inline-flex items-center gap-1.5 text-[12px] text-text-secondary hover:text-foreground">
              <ArrowLeft className="h-3 w-3" /> Specga qaytish
            </Link>
            <h1 className="mt-2 text-[24px] font-semibold tracking-tight">Drift hisobotlari · {spec.name}</h1>
            <p className="mt-1 text-[13px] text-text-secondary">Sinovchi o'zgarishlarni jo'natishdan oldin aniqlash uchun spec versiyalarini taqqoslang.</p>
          </div>
          <button onClick={() => setShowCompare((v) => !v)} className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-3.5 text-[13px] font-medium text-white hover:bg-primary-hover">
            <GitCompare className="h-3.5 w-3.5" /> Yangi versiyani taqqoslash
          </button>
        </div>

        {showCompare && (
          <div className="rounded-xl border border-border bg-surface p-5 space-y-3">
            <div className="text-[14px] font-semibold">Yangilangan specni yuklash</div>
            <label className="flex items-center justify-center gap-2 rounded-md border border-dashed border-border bg-background/40 py-6 cursor-pointer hover:border-primary/50">
              ...
              <span className="text-[13px] text-text-secondary">Yuklash uchun bosing .yaml / .json</span>
            ...
            <textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} spellCheck={false}
              placeholder="…yoki yangi spec versiyasini qo'ying"
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowCompare(false)} className="h-9 rounded-md border border-border bg-surface px-3 text-[13px] hover:border-border-hover">Bekor qilish</button>
              <button disabled={busy} onClick={compare} className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-3.5 text-[13px] font-medium text-white hover:bg-primary-hover disabled:opacity-60">
                {busy && <Loader2 className="h-3.5 w-3.5 animate-spin" />} Drift tekshiruvini ishga tushirish
              </button>
            </div>
          </div>
        )}
        ...
        {!reports.length ? (
          <div className="rounded-xl border border-dashed border-border bg-surface/40 p-12 text-center">
            ...
            <h2 className="mt-4 text-[16px] font-semibold">Hali drift hisobotlari yo'q</h2>
            <p className="mt-1 text-[13px] text-text-secondary">Sinovchi o'zgarishlarni aniqlash uchun bu specning yangi versiyasini yuklang.</p>
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
            <aside className="rounded-xl border border-border bg-surface overflow-hidden">
              <div className="border-b border-border px-4 py-2.5 text-[12px] font-semibold">Tarix</div>
              <ul>
                {reports.map((r) => (
                  <li key={r.id}>
                    <button
                    ...
                    >
                      ...
                      <div className="mt-1.5 flex gap-1.5 text-[10px]">
                        {r.breaking_count > 0 && <span className="rounded bg-danger/15 px-1.5 py-0.5 text-danger">{r.breaking_count} sinovchi</span>}
                        {r.warning_count > 0 && <span className="rounded bg-warning/15 px-1.5 py-0.5 text-warning">{r.warning_count} ogohl.</span>}
                        {r.info_count > 0 && <span className="rounded bg-blue-500/15 px-1.5 py-0.5 text-blue-400">{r.info_count} ma'lumot</span>}
                      </div>
                    </button>
                  </li>
                ))}
            ...
            <section className="space-y-5">
              {report ? (
                <>
                  <Group title="Sinovchi o'zgarishlar" tone="danger" Icon={AlertCircle} items={breaking} />
                  <Group title="Ogohlantirishlar" tone="warning" Icon={AlertTriangle} items={warnings} />
                  <Group title="Ma'lumot" tone="info" Icon={Info} items={info} />
                  {!changes.length && <div className="rounded-xl border border-success/30 bg-success/10 p-5 text-[13px] text-success">O'zgarishlar aniqlanmadi — specslar bir xil.</div>}
                </>
              ) : (
                <div className="text-text-secondary">Hisobotni tanlang</div>
              )}
            </section>
          </div>
        )}
