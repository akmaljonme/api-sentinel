...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Flowt — API flows never break production" },
      { name: "description", content: "API contract testing platform. Live mock servers, drift detection, and CI/CD alerts." },
      { property: "og:title", content: "Flowt — API flows never break production" },
      { property: "og:description", content: "Live mock servers. Drift detection. CI/CD alerts." },
    ],
  }),
  component: Landing,
});
      ...
      <div className="relative mx-auto max-w-7xl px-6 text-center">
        ...
        <div className="mt-10 flex flex-col items-center gap-4 animate-fade-up" style={{ animationDelay: "240ms" }}>
          <div className="flex items-center gap-3">
            ...
            <span className="text-[13px] text-text-secondary">Built for API teams using these tools</span>
          </div>
...
function ProblemSolution() {
  const probs = [
    ...
    "Manual Postman collections, always out of date",
  ];
  const sols = [
    "On-the-fly mock server from any OpenAPI spec",
    "Drift detection catches changes before merge",
    "Auto-sync from GitHub, always fresh",
  ];
  return (
      ...
      <div className="mx-auto max-w-7xl px-6 grid gap-16 lg:grid-cols-2 lg:items-center">
        ...
        <div className="order-1 lg:order-2 max-w-lg">
          ...
          <h3 className="mt-4 text-[32px] sm:text-[40px] font-bold tracking-tight leading-[1.1]">
            From spec to working mock server in <span className="text-gradient">4 seconds</span>
          </h3>
          ...
          <ul className="mt-6 space-y-3">
            {[
              ...
              "Custom scenarios with a single click",
            ].map((b) => (
    ...
    <section className="border-t border-border py-32">
      <div className="mx-auto max-w-7xl px-6 grid gap-16 lg:grid-cols-2 lg:items-center">
        <div className="max-w-lg">
          ...
          <h3 className="mt-4 text-[32px] sm:text-[40px] font-bold tracking-tight leading-[1.1]">
            Breaking changes are <span className="text-gradient">flagged before merge</span>
          </h3>
          <p className="mt-4 text-[16px] text-text-secondary">
            Flowt compares every uploaded contract against the current spec. Breaking changes
            are saved as actual drift reports. Safe additions are shipped freely.
          </p>
          <div className="mt-6 rounded-xl border border-border bg-surface overflow-hidden">
            <div className="flex items-center gap-2 border-b border-border px-3 py-2 text-[12px] text-text-secondary">
              ...
              <span>left a comment on PR #1042 · just now</span>
            </div>
            <div className="p-4 space-y-3 text-[13px]">
              ...
              <div className="rounded-md bg-danger/10 border border-danger/20 p-3 font-mono text-[12px]">
                <div className="text-danger">+ POST /orders customer_id required</div>
                <div className="text-text-muted">  field added to required[]</div>
              </div>
              <div className="text-text-secondary">3 frontend components and 12 tests use these endpoints. See impact →</div>
            </div>
          </div>
        </div>
        <DiffVisual />
...
`}
        ...
        <div className="max-w-lg">
          ...
          <h3 className="mt-4 text-[32px] sm:text-[40px] font-bold tracking-tight leading-[1.1]">
            One line in your pipeline. <span className="text-gradient">No surprises.</span>
          </h3>
          <p className="mt-4 text-[16px] text-text-secondary">
            Use Flowt in GitHub Actions, GitLab CI, CircleCI, or Buildkite.
            Compare uploaded contracts and save rich reports in your workspace.
          </p>
...
function Testimonials() {
  const ts = [
    { i: "AC", n: "Alex Chen", r: "CTO @ Linear", g: "from-indigo-500 to-cyan-500", q: "We caught breaking changes before our frontend team could ship against the wrong contract." },
    { i: "SK", n: "Sarah Kim", r: "Staff Eng @ Stripe", g: "from-rose-500 to-orange-500", q: "Finally a contract testing tool that doesn't feel like it's from 2014. Linear-level UX makes it genuinely fun." },
    { i: "MJ", n: "Marcus Johnson", r: "Frontend Lead @ Vercel", g: "from-emerald-500 to-teal-500", q: "4-second mock servers isn't marketing fluff — we measured it. Unblocked our team in the first week." },
  ];
  return (
    ...
    <section className="border-t border-border py-32">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-text-secondary">Start free, upgrade as your team grows. Cancel anytime.</p>
          <div className="mt-8 inline-flex items-center gap-1 rounded-full border border-border bg-surface p-1">
            {(["Monthly", "Yearly"] as const).map((m) => {
              const on = (m === "Yearly") === annual;
              return (
                <button
                  key={m}
                  onClick={() => setAnnual(m === "Yearly")}
                ...
                >
                  {m} {m === "Yearly" && <span className={`ml-1 text-[10px] ${on ? "text-white/80" : "text-success"}`}>−20%</span>}
                </button>
              );
            })}
          </div>
          ...
          {tiers.map((t) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-1 text-[12px] text-text-muted">{annual ? "Billed yearly" : "Billed monthly"}</p>
                <button
...
function FAQ() {
  const qs = [
    ["Do I need to write tests?", "No. Flowt derives endpoints, mock responses, and drift reports from your OpenAPI YAML or JSON file."],
    ["Which spec formats are supported?", "OpenAPI 3.0, 3.1, Swagger 2.0, and GraphQL SDL. Import Postman collections on the Pro plan."],
    ["Can I use Flowt with private APIs?", "Yes. Upload specs from private repos or CI; mock servers and reports stay within your workspace."],
    ["How does drift detection work?", "Every commit compares the new spec against the last published contract. Breaking changes are blocked; safe changes pass."],
    ["Which languages do mock clients support?", "TypeScript, Python, Go, Ruby, PHP, Rust, Java, Kotlin, Swift, and C#. Generated SDKs come with full types."],
    ["Is there a free plan?", "Yes — always free for 1 spec, 1 mock server, and 10K requests/mo. No credit card required."],
  ];
  const [open, setOpen] = useState<number | null>(0);
  return (
    <section className="border-t border-border py-32">
      <div className="mx-auto max-w-3xl px-6">
        <h2 className="text-center text-[36px] sm:text-[44px] font-bold tracking-tight">Questions & Answers.</h2>
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-primary/15 via-surface to-cyan-500/10 p-12 text-center">
          ...
          <div className="relative">
            <h2 className="text-[36px] sm:text-[44px] font-bold tracking-tight">
              Ship APIs <span className="text-gradient">that never break</span>
            </h2>
            <p className="mx-auto mt-3 max-w-md text-text-secondary">Start free in 30 seconds. No credit card required. Cancel anytime.</p>
