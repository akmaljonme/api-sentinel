...
export function CommandPalette({ onNewSpec }: { onNewSpec: () => void }) {
  ...
  const items: Item[] = [
    ...
    { id: "home", label: "Go to Dashboard", group: "Navigate", icon: Home, run: () => { nav({ to: "/dashboard" }); setOpen(false); } },
    { id: "settings", label: "Open Settings", hint: "Team, API keys", group: "Navigate", icon: Settings, run: () => { nav({ to: "/settings" }); setOpen(false); } },
    ...specs.map((s) => ({
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center px-4 pt-[12vh]" onClick={() => setOpen(false)}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-t border-border bg-background/40 px-4 py-2 text-[11px] text-text-muted">
          <div className="flex items-center gap-1.5"><Sparkles className="h-3 w-3" /> Flowt Command</div>
